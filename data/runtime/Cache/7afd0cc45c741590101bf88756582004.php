...
<?php if(is_array($arrFollowUser)): foreach($arrFollowUser as $key=>$item): ?><li class="clearfix" id="u<?php echo ($item[userid]); ?>">
        ...
        <div class="info">
          <h3> <a title="<?php echo ($item[username]); ?>" href="<?php echo U('space/index/index',array('id'=>$item[doname]));?>"><?php echo ($item[username]); ?></a></h3>
          <!-- 签名档 -->
...
$('.user-list .ban').click(function () {
	...
	if (hasBlackList) {
		$(this).parents('li').fadeOut(function () {
			var posturl = "<?php echo U('public/user/unfollow',array('d'=>'user_nofollow_ajax'));?>";
			$.post_withck(
				posturl,
				{ 'userid': peopleId },
				function (res) {
...
<?php if($strUser[userid] == $visitor[userid]): ?><p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('public/user/followed',array('userid'=>$strUser[userid]));?>">关注我的人(<?php echo ($strUser[count_followed]); ?>)</a>
        </p> 
<?php else: ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('public/user/followed',array('userid'=>$strUser[userid]));?>">关注<?php echo ($strUser[username]); ?>的人(<?php echo ($strUser[count_followed]); ?>)</a>
        </p><?php endif; ?>               
<!---
        <p class="pl2">
