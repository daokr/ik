...
<h2><?php echo ($title); ?></h2>
    <div>
    <form action="<?php echo U('admin/setting/url');?>" method="post">
    <table>
        <tr>
            <td width="100">形式1：</td>
            <td>
            <?php if($config[URL_MODEL] == 0): ?><label><input type="radio" name="url_model" value="0" checked="select"/> http://www.ikphp.com/index.php?app=article&m=article&a=show&id=1 </label>
            <?php else: ?>
            <label><input type="radio" name="url_model" value="0" /> http://www.ikphp.com/index.php?app=article&m=article&a=show&id=1 </label><?php endif; ?>
            </td>
        </tr>
        <tr>
            <td>形式2：</td>
