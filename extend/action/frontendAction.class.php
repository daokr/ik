...
class frontendAction extends baseAction {
	...
	protected  function _nav($app_name){
		if (! empty ( $app_name )) {
			...
			switch ($app_name) {
				...
				case "site" :
					// 小站
					$arrNav['index'] = array('name'=>'小站首页', 'url'=>U('site/index/index'));
					if($this->visitor->info['userid']){
						$arrNav['mysite'] = array('name'=>'我的小站', 'url'=>U('site/index/mysite'));
					}
					$arrNav['explore'] = array('name'=>'发现小站', 'url'=>U('site/index/explore'));
					break;										
				default:
	...
	protected  function _navlogo($app_name){
		if (! empty ( $app_name )) {
			...
			switch ($app_name) {
				case "group" :
					$arrLogo = array('name'=>'小组', 'url'=>U('group/index/index'), 'style'=>'site_logo nav_logo');
					break;
				case "mall" :
					$arrLogo = array('name'=>'商城', 'url'=>U('mall/index/index'), 'style'=>'site_logo nav_logo');
					break;						
				case "article" :
					$arrLogo = array('name'=>'阅读', 'url'=>U('article/index/index'), 'style'=>'site_logo nav_logo');
					break;
				default:
